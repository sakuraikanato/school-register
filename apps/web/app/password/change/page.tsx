    ...
    <main className="password-page">
          ...
          <div className="password-change-fields">
            <label>
              ...
              <span className="password-change-input">
                <input name="password" type={showPassword ? "text" : "password"} placeholder="新しいパスワードを入力" />
                <button type="button" aria-label={showPassword ? "パスワードを隠す" : "パスワードを表示"} onClick={() => setShowPassword((value) => !value)}><PasswordVisibilityIcon visible={showPassword} /></button>
              </span>
            </label>
            <label>
              新しいパスワード（確認）
              <span className="password-change-input">
                <input name="confirmation" type={showConfirmation ? "text" : "password"} placeholder="新しいパスワードを再入力" />
                <button type="button" aria-label={showConfirmation ? "確認用パスワードを隠す" : "確認用パスワードを表示"} onClick={() => setShowConfirmation((value) => !value)}><PasswordVisibilityIcon visible={showConfirmation} /></button>
              </span>
            </label>
          </div>
      ...
      <section className="password-change-info" aria-label="パスワードの条件">
        ...
        <div className="password-change-rules">
          <h3>パスワードの条件</h3>
          <ul>
            <li><span aria-hidden="true">✓</span>8文字以上</li>
            <li><span aria-hidden="true">✓</span>英数字を含む</li>
            <li><span aria-hidden="true">✓</span>大文字を含む</li>
            <li><span aria-hidden="true">✓</span>小文字を含む</li>
          </ul>
        </div>
      </section>
    </main>
