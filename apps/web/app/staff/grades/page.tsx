...
function Content() {
  ...
  return <>{(dashboard.loading || entry.loading) && <RpcStateMessage loading error={null} />}{(dashboard.error || entry.error) && <RpcStateMessage loading={false} error={dashboard.error ?? entry.error} />}{entry.data && <><div className="staff-grade-desktop">{isDeveloperMode && <DeveloperGradePeriodControls subjectId={subjectId} value={period!} />}<StaffGradeScreen key={`${subjectId}-${period!.yearId}-${period!.term}`} data={entry.data} subjectId={subjectId} query={query} /></div><div className="mobile-only"><StaffGradeMobile data={entry.data} /></div></>}</>;
}
